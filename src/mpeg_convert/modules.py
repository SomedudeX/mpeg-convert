...
from .utils import Logger, ProgramInfo
from .arguments import ArgumentsError, parse_arguments
...
def init_logging(argv_properties: Dict[str, Any], argv: List[str]) -> int:
    ...
    log.debug(f"Mpeg-convert {ProgramInfo.VERSION}")
...
def start(argv: List[str]) -> int:
    ...
    Notes:
        ...
        such as ArgumentsError and uncaught exceptions (so as to facilitate a
